...
point4 vertices[8] = {
    point4( -0.5, 3.0,  0.5, 1.0 ),
    point4( -0.5,  4.0,  0.5, 1.0 ),
    point4(  0.5,  4.0,  0.5, 1.0 ),
    point4(  0.5, 3.0,  0.5, 1.0 ),
    point4( -0.5, 3.0, -0.5, 1.0 ),
    point4( -0.5,  4.0, -0.5, 1.0 ),
...
display( void )
{
    ...
    // Perspective
    mat4 p = Perspective( 70.0, 1.0, 6.0, 100.0 );
    glUniformMatrix4fv( projection, 1, GL_TRUE, p );

    // Model View, hér er hinsvegar vandamál með LookAt, ergó Translate
    mat4 mv;
    mv *= Translate( curr_x, curr_y, -curr_z*6 );
    ...
    //Búkur
	mv *= Translate(0.0,-5.0, 0.0);
	mv *= Scale(1.5, 2.0, 1.2);
    glUniformMatrix4fv( modelview, 1, GL_TRUE, mv );
    glDrawArrays( GL_TRIANGLES, 0, NumVertices );

	//Handleggur
	mv *= Translate(0.9, 3.1, 0.0);
	mv *= Scale(1.3, 0.2, 0.4);
    glUniformMatrix4fv( modelview, 1, GL_TRUE, mv );
    glDrawArrays( GL_TRIANGLES, 0, NumVertices );

	//Annar handleggur
	mv *= Translate(-1.4, 0.0, 0.0);
	glUniformMatrix4fv( modelview, 1, GL_TRUE, mv );
    glDrawArrays( GL_TRIANGLES, 0, NumVertices );

	//Fótleggur
	mv *= Scale(0.2, 5, 1.0);
	mv *= Translate(2.5, -4.0, 0.0);
	glUniformMatrix4fv( modelview, 1, GL_TRUE, mv );
    glDrawArrays( GL_TRIANGLES, 0, NumVertices );
